  ...
  <div :dir="dir" class="v-select" :class="stateClasses">
    ...
    <div :id="`vs${uid}__combobox`" ref="toggle" @mousedown="toggleDropdown($event)" class="vs__dropdown-toggle" role="combobox" :aria-expanded="dropdownOpen.toString()" :aria-owns="`vs${uid}__listbox`" aria-label="Search for option">
      ...
      <div class="vs__selected-options" ref="selectedOptions">
        <slot v-for="(option, index) in selectedValue"
              name="selected-option-container"
              :option="normalizeOptionForSlot(option)"
              :deselect="deselect"
              :multiple="multiple"
              :disabled="disabled">
          <span :key="getOptionKey(option)" :class="{vs__selected:true, vs__selected_hidden: index > 1} ">
        ...
        <slot name="search" v-bind="scope.search">
          <input class="vs__search" v-bind="scope.search.attributes" v-on="scope.search.events" @focus="focusOnInput=true" :readonly="!focusOnInput">
        </slot>
      </div>
      ...
      <div class="vs__actions" ref="actions">
        ...
        <slot name="spinner" v-bind="scope.spinner">
          <div class="vs__spinner" v-show="mutableLoading">Загрузка...</div>
        </slot>
      </div>
    </div>
    <button class="vs__button__collapse" @click="changeCollapse()" v-if="countSelectedValues > 2"> 
      <span  v-if="collapse" >еще + {{countSelectedValues - 2}} <i class="fas fa-arrow-down"></i></span>
      <span v-if="!collapse"><i class="fas fa-arrow-up"></i></span>
          </button>
    <div class="vs__hidden" v-if="!collapse">
      <slot v-for="option in selectedValue"
              ...
              :multiple="multiple"
              :disabled="disabled">
          <span :key="getOptionKey(option)" :class="{vs__selected:true} ">
            <slot name="selected-option" v-bind="normalizeOptionForSlot(option)">
              {{ getOptionLabel(option) }}
            </slot>
            <button v-if="multiple" :disabled="disabled" @click="deselect(option)" type="button" class="vs__deselect" :title="`Deselect ${getOptionLabel(option)}`" :aria-label="`Deselect ${getOptionLabel(option)}`" ref="deselectButtons">
    ...
    <transition :name="transition">
      <ul ref="dropdownMenu" v-if="dropdownOpen" :id="`vs${uid}__listbox`" class="vs__dropdown-menu" role="listbox" @mousedown.prevent="onMousedown" @mouseup="onMouseUp" v-append-to-body>
        ...
        <li v-if="filteredOptions.length === 0" class="vs__no-options">
          <slot name="no-options" v-bind="scope.noOptions">Извините, нет подходящих</slot>
        </li>
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      changeCollapse(){
        return this.collapse = this.collapse ? false : true;
      },
      /**
