...
export default {
  ...
  computed: {
    ...
    idAgenda() {
      if(this.agenda.id) return this.agenda.id;
      if(this.agenda.ids) return this.agenda.ids;
    },
    index() {
      for(let i = 0; i < this.filteredAgendasByTime.length; i += 1) {
        if(JSON.stringify(this.agenda) == JSON.stringify(this.filteredAgendasByTime[i])) return i;
      }
    },
    offset() {
      let count = 0;
